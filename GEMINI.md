...
You are building AISRS-Japanese, a specialized AI assistant for Japanese language learning. Your purpose is to help me, your user, build a personalized, AI-driven spaced repetition system (SRS).
...
## Workflow Rules for codebase changes:
...
* **Linting & Testing:** Use Biome for linting. While we have the infrastructure to use Playwright for testing, this is not needed at this time.
...
* **Lesson:** A Gemini generated learning aid for a specific learning item / goal

* **AI Generated Question:** A Gemini generated question for a specific learning item / goal

* **Context Example:** A Gemini generated sentence that demonstrates the use of a learning item. This may be expanded to other kinds of context examples

* **Component Kanji:** A Kanji that forms part of an existing learning item. It's a KU in and off itself and can potentially be learned independently
...
* **Content Library:** This is where generated resources can be stored permanently as Markdown files. While the app does support management of these files through the /library endpoint, these are not the source of truth for resources used with the app
...
*
* **Library:** Not implemented as a button currently but can be found via the `/library` URL path
...
Clicking on the Review button will iterate through the past due review facets. For the XX-to-YY facets the questions are displayed directly (with no external API calls). When the user answers, that response is checked against values derived from the KU and/or the Lesson and if there's no match, it's sent with a prompt to the Gemini API which checks the answer and gives feedback. Gemini decides if the question is wrong or right. If the question is marked as wrong it is re-queued. At the moment this happens a fixed number of times before the nextReviewAt date/time is bumped to the future. AI Generated questions, are just that, the question (along with the expected and alternative answers) is generated by Gemini and the answer is reviewed by Gemini.
...
* **Enriched Lessons:** Implemented furigana handling and display in lessons

* **Usability**: Implemented use of `wanakana` library to convert input into Japanese Kana when answering "content-to-reading" type lessons.

* **Content Library**: Built a content service to display learning resources. The plan was to use this as single source of truth but now it's just something that can be generated manually.
...
* **Phase 4 (Testing and bug squashing):** Complete

* **Phase 5** (Kanji Refactor): See below

* **Phase 6 (Question Persistence and tracking):** TBD
...
**1. Implement Kanji-to-Vocab "Tieback" (PENDING)**
...
**2. Implement Selective Reading Facets on the Kanji Learn Page (IMPLEMENTED)**
...
**3. Update the Review Facet API (IMPLEMENTED)**
