...
export class UserConceptsController {
  ...
  async enroll(@UserId() uid: string, @Body('conceptId') conceptId: string) {
    ...
    return this.conceptsService.enroll(uid, conceptId);
  }

  @Get()
  async findAll(@UserId() uid: string) {
    this.logger.log(`GET /user-concepts — uid=${uid}`);
    const results = await this.conceptsService.findAllForUser(uid);
  ...
  async getFacets(@UserId() uid: string, @Param('conceptId') conceptId: string) {
    ...
    return this.conceptsService.getFacets(uid, conceptId);
  }
  ...
  async createFacets(
    ...
    return this.conceptsService.createFacets(uid, conceptId, mechanicIndices, includeAiQuestion);
  }
}
