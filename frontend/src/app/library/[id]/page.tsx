    ...
    <main className="container mx-auto max-w-5xl p-8">
      ...
      <header className="mb-8 border-b border-shodo-ink/10 pb-6">
        ...
        <div className="bg-shodo-paper-warm p-6 rounded border border-shodo-ink/10 shadow-sm">
          {topic.reading && (
            <p className="text-2xl text-shodo-indigo font-mono mb-2">
              {topic.reading}
            </p>
          )}
          {topic.definition && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <section className="bg-shodo-paper border border-shodo-ink/10 p-5 rounded-lg shadow-sm">
            <h3 className="text-sm font-bold text-shodo-ink-light uppercase tracking-wider mb-4">
              Metadata
            </h3>
            <dl className="space-y-3 text-sm">
              {topic.partOfSpeech && (
                <div>
                  ...
                  <dd className="text-shodo-ink font-mono font-medium">
                    {topic.partOfSpeech}
                  </dd>
                </div>
              )}
              ...
              {topic.grammarFunction && (
                <div>
                  ...
                  <dd className="text-shodo-ink font-medium">
                    {topic.grammarFunction}
                  </dd>
                </div>
              )}
          ...
          <section>
            ...
            {lessons.length === 0 ? (
              <p className="text-shodo-ink-faint italic">
                No lessons linked to this topic.
              </p>
            ) : (
              <div className="grid gap-4">
                {lessons.map((lesson) => (
                  <div
                    key={lesson.id}
                    className="bg-shodo-paper p-5 rounded-lg border border-shodo-ink/10 hover:border-shodo-matcha transition-colors group shadow-sm"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-shodo-ink-light font-mono">
                          {lesson.id}
                        </p>
                      </div>
                      <Link
            ...
            {sentences.length === 0 ? (
              ...
              <div className="space-y-4">
                {sentences.map((sentence) => (
                  <div
                    key={sentence.id}
                    className="bg-shodo-paper p-6 rounded-lg border-l-4 border-shodo-indigo shadow-sm relative overflow-hidden"
                  >
                    ...
                    <div className="mt-2 pt-2 flex gap-2 text-xs text-shodo-ink-faint">
                      ...
                      {sentence.tags?.map((tag) => (
                        <span
                          key={tag}
                          className="text-shodo-indigo opacity-75"
                        >
                          #{tag}
                        </span>
                      ))}
