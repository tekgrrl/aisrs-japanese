...
interface ReviewFacetClient
  extends Omit<
    ReviewFacet,
    "nextReviewAt" | "lastReviewAt" | "createdAt" | "history"
  > {
  nextReviewAt: string;
  lastReviewAt?: string;
  createdAt: string;
...
export default function AdminPage() {
  const [activeTab, setActiveTab] = useState<
    "knowledge-units" | "review-facets" | "bulk-import"
  >("knowledge-units");
  ...
  const [facetSortBy, setFacetSortBy] = useState<
    "srsStage" | "nextReviewAt" | "kuId"
  >("kuId");
  ...
  useEffect(() => {
    const fetchKus = async () => {
      try {
        const res = await apiFetch("/api/knowledge-units/get-all");
  ...
  useEffect(() => {
    if (activeTab === "review-facets" && !facetLoaded && !loadingFacets) {
      const fetchFacets = async () => {
        ...
        try {
          const res = await apiFetch("/api/reviews/facets");
          const data = await res.json();
  ...
  const sortedFacets = [...reviewFacets].sort((a, b) => {
      ...
      try {
        ...
        if (file.name.endsWith(".json")) {
          ...
          if (Array.isArray(json.items)) {
            ...
            throw new Error(
              "Invalid JSON format. Expected { items: string[] } or string[]",
            );
          }
        } else {
        ...
        const res = await apiFetch("/api/lessons/batch", {
          method: "POST",
        ...
        const data = await res.json();
        setImportStatus(
          `Success! Batch processing started for ${data.count} items.`,
        );
      } catch (error) {
          ...
          {loadingFacets ? (
            ...
            <>
              <div className="flex justify-end mb-4 gap-2">
                ...
                <button
                  onClick={() => handleFacetSort("srsStage")}
                  className="text-sm text-gray-600 hover:text-black"
                >
                  Sort by SRS{" "}
                  {facetSortBy === "srsStage" &&
                    (facetSortOrder === "asc" ? "▲" : "▼")}
                </button>
              </div>
              <div className="overflow-x-auto shadow rounded-lg border">
                    ...
                    {sortedFacets.map((facet) => {
                      ...
                      return (
                        <tr key={facet.id} className="hover:bg-gray-50">
                          <td className="py-2 px-6 whitespace-nowrap font-medium">
                            {getKuContent(facet.kuId)}
                          </td>
                          <td className="py-2 px-6 whitespace-nowrap text-sm text-gray-600">
                            {facet.facetType}
                          </td>
        ...
        <div className="max-w-xl">
          <div className="bg-white p-6 rounded-lg shadow border">
            ...
            <p className="text-gray-600 mb-4">
              Upload a JSON file (
              <code>&#123; "items": ["word1", "word2"] &#125;</code>) or a
              CSV/Text file (one word per line). These items will be queued for
              lesson generation in the background.
            </p>
            ...
            {importStatus && (
              <div
                className={`p-4 rounded-md ${importStatus.startsWith("Error") ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
              >
                {importStatus}
              </div>
            )}
