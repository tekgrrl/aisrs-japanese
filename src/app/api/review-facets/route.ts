...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (dueOnly) {
      ...
      const kuPromises = kuIds.map((id) =>
        db.collection(KNOWLEDGE_UNITS_COLLECTION).doc(id).get(),
      );
      const kuDocs = await Promise.all(kuPromises);

      const kus = kuDocs.reduce(
        (acc, doc) => {
          if (doc.exists) {
      ...
      const lessonPromises = kuIds.map((id) =>
        db.collection(LESSONS_COLLECTION).doc(id).get(),
      );
      const lessonDocs = await Promise.all(lessonPromises);

      const lessons = lessonDocs.reduce(
        (acc, doc) => {
          if (doc.exists) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.info(
      `Starting batch for ${facetsToCreate.length} facets for KU ${kuId}`,
      { facetsToCreate },
    );
    // --- END DEBUG ---
    ...
    for (const facet of facetsToCreate) {
      ...
      // --- New: Handle single Kanji component key ---
        ...
        facetKey.startsWith("Kanji-Component-") &&
        facetKey.split("-").length === 3
      ) {
        const kanjiChar = facetKey.split("-")[2];
        logger.debug(
          `Handling consolidated Kanji Component key for: '${kanjiChar}'`,
        );
      ...
      // --- Complex Kanji Facets (DEPRECATED but kept for safety) ---
        ...
        logger.debug(
          `Extracted Kanji Component: char='${kanjiChar}', type='${facetType}' from key='${facetKey}'`,
        );
        // --- END DEBUG ---
        ...
        logger.debug(
          `Kanji find query result for '${kanjiChar}': empty=${kanjiSnapshot.empty}, size=${kanjiSnapshot.size}`,
        );
        // --- END DEBUG ---
        ...
        if (kanjiSnapshot.empty) {
          // --- DEBUG ---
          logger.info(
            `Kanji KU not found (snapshot empty), proceeding to CREATE new one for: ${kanjiChar}`,
          );
          // --- END DEBUG ---
          ...
          try {
            if (!genAI) {
              ...
              if (!apiKey) {
                throw new Error(
                  "GEMINI_API_KEY is not defined for Kanji creation",
                );
              }
            ...
            apiLogRef = await db
              ...
              .add(initialApiLogData);
            logger.debug(
              `Created initial Kanji Gen log entry: ${apiLogRef.id}`,
            );
            // --- End Log ---
            ...
            try {
              ...
              logger.error("Failed to parse AI JSON for new Kanji", {
                text: apiAiJsonText,
                parseError,
              });
            ...
            if (apiLogRef) {
              ...
              const apiUpdateData: Partial<ApiLog> = {
              ...
              if (apiErrorOccurred) {
                ...
                if (apiCapturedError instanceof Error) {
                  errorDetails = {
                    message: apiCapturedError.message,
                    stack: apiCapturedError.stack,
                  };
                } else {
                  try {
                    errorDetails = {
                      rawError: JSON.stringify(apiCapturedError, null, 2),
                    };
                  } catch {
                ...
                apiUpdateData.responseData = {
                  rawText: apiAiJsonText,
                  parsedJson: apiKanjiData,
                };
              }
              try {
                await apiLogRef.update(apiUpdateData);
                logger.debug(`Updated Kanji Gen log entry: ${apiLogRef.id}`);
              } catch (logUpdateError) {
                logger.error(
                  `Failed to update Kanji Gen log entry ${apiLogRef.id}`,
                  { logUpdateError },
                );
              }
            }
          ...
          // --- DEBUG ---
          logger.debug(
            `BATCH SET new Kanji KU: '${kanjiChar}' (ID: ${kanjiKuId})`,
          );
          // --- END DEBUG ---
        } else {
          ...
          // --- DEBUG ---
          logger.debug(
            `FOUND existing Kanji KU: '${kanjiChar}' (ID: ${kanjiKuId})`,
          );
          // --- END DEBUG ---
        }
        ...
        // --- DEBUG ---
        logger.debug(
          `BATCH SET complex facet: '${facetType}' for Kanji KU ${kanjiKuId}`,
        );
        // --- END DEBUG ---
      } else {
    ...
    if (parentFacetCount > 0) {
      const parentKuRef = db.collection(KNOWLEDGE_UNITS_COLLECTION).doc(kuId);
      ...
      logger.debug(
        `BATCH UPDATE parent KU ${kuId}: status to reviewing, facet_count to ${parentFacetCount}`,
      );
    }
    ...
    return NextResponse.json(
      { message: "Facets created successfully" },
      { status: 201 },
    );
  } catch (error) {
